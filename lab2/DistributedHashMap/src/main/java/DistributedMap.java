...
public class DistributedMap extends ReceiverAdapter implements SimpleStringMap {
    ...
    public DistributedMap(JChannel communicationChannel) {
    ...
    private void putNotify(String key, Integer value) {
        ...
        Message message = new Message(null, null, notification);
        try {
    ...
    private static class MergeViewHandler extends Thread {
        ...
        public void run() {
            ...
            if (!primaryPartitionView.getMembers().contains(localAddress)) {
                System.out.println("Nie jestes czlonkiem primary partition (" + primaryPartitionView + "), ustalisz stan od nowa");
                try {
                ...
                System.out.println("Jestes czlonkiem primary partition (" + primaryPartitionView + "), nic nie zrobisz");
            }
        }
    }
